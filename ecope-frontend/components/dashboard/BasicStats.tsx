...
export default function BasicStatsDisplay({ stats, loading }: BasicStatsProps) {
  ...
  // Handle both possible key formats: "Open" and "Pending" are typical statuses
  const openComplaints = (stats?.by_status?.["Open"] || stats?.by_status?.["Pending"] || 0);
  
  // High and Critical urgency levels
  const highUrgency = (stats?.by_urgency?.["High"] || stats?.by_urgency?.["Urgency.HIGH"] || 0);
  const criticalUrgency = (stats?.by_urgency?.["Critical"] || stats?.by_urgency?.["Urgency.CRITICAL"] || 0);
  const totalHighCritical = highUrgency + criticalUrgency;
  
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <Card className="border-l-4 border-l-blue-500 hover:shadow-md transition-shadow">
        <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0 text-muted-foreground/80">
      ...
      <Card className="border-l-4 border-l-amber-500 hover:shadow-md transition-shadow">
        ...
        <CardContent>
          ...
          {stats && totalComplaints > 0 && (
            <div className="text-xs text-muted-foreground mt-1">
              {((openComplaints / totalComplaints) * 100).toFixed(1)}% of total complaints
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="border-l-4 border-l-red-500 hover:shadow-md transition-shadow">
        ...
        <CardContent>
          ...
          {stats && totalComplaints > 0 && (
            <div className="text-xs text-muted-foreground mt-1">
              {((totalHighCritical / totalComplaints) * 100).toFixed(1)}% High/Critical
            </div>
          )}
        </CardContent>
      </Card>
